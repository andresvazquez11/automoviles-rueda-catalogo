...
| `pruebas.json` + `pruebas.py` | Bloque "Míralo en acción · pruebas en vídeo" (final de la ficha): vídeos REALES de YouTube por MODELO, cada id comprobado con oEmbed, filtros `desde`/`hasta`/`version_contiene`/`version_empieza` como `prensa.json`. Se edita a mano |
