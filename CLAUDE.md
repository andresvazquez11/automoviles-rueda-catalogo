...
| `generar_og_portada.py` | Imagen de previsualización al compartir el enlace de la portada (WhatsApp, estados, Facebook): `assets/og-portada-{perfil}.jpg`, 1200×630, una por asesor con nombre y teléfono, usando `assets/hero-portada.jpg` y las fuentes de `assets/fonts/`. Se ejecuta A MANO si cambia la foto o un asesor; `generar_web.py` (og_portada_html) solo la enlaza |
| `recortar_portadas.py` | Recorta el coche de la foto de portada (rembg, sin fondo) → `web_fotos/recortes/{id}.webp` + `recortes.json` (huella de la portada; solo se recorta de nuevo si cambia). Lo lanza `generar_web.py` en un proceso aparte tras copiar las fotos. Efecto en el catálogo: al entrar con el ratón en la foto, el coche recortado sale más grande por encima de todo; al mover el ratón vuelve y sigue el paso de fotos. Solo en ordenador. Una tarjeta solo lleva el efecto si el recorte es de su portada ACTUAL |
...
| `confianza_dwa.py` | Sello "Das WeltAuto · Concesionario oficial" + botones Coches/Quiénes somos del header, pastilla de garantía REAL en cada tarjeta, banners entre filas (los reparte `assets/confianza.js`, llamado desde `aplicar()` del filtro) y tarjeta "Comprando este coche en Das WeltAuto" al final de la ficha. Acordado con Andrés: NO publicar "hasta 24 meses", "15 días / 1.000 km" ni garantía de batería |
| `pruebas.json` + `pruebas.py` | Bloque "Míralo en acción · pruebas en vídeo" (final de la ficha): vídeos REALES de YouTube por MODELO, cada id comprobado con oEmbed, filtros `desde`/`hasta`/`version_contiene`/`version_empieza` como `prensa.json`. Los que llevan `"auto"` los añadió `buscar_pruebas.py` |
| `buscar_pruebas.py` | AUTOMÁTICO en cada actualización (GitHub Actions, antes de generar la web): por cada modelo+año del catálogo sin vídeos, busca "prueba {modelo} {año} review español" en YouTube, filtra (nombra el modelo, es prueba/review, sin Shorts/comparativas, publicado entre año-3 y año, sin "Nuevo…" de la generación siguiente, medios conocidos primero), verifica con oEmbed y añade hasta 3 a `pruebas.json` (desde/hasta = año±1). Lo apunta en el correo. Sin resultados → no reintenta hasta 7 días (`_buscados`) |
| `comparador.py` + `assets/comparador.js` | Comparador completo de la portada: `main()` escribe `comparador.json` (ficha técnica de cada coche por id de tarjeta) y el JS lo descarga al pulsar "Ver comparación"; tabla por grupos con ★ al mejor dato |
| `quienes_somos.py` | Página `/quienes-somos/` (y `/alejandro/quienes-somos/`): historia, sedes con fotos de `assets/sedes/` y taller propio de Guadalhorce. Las fotos actuales son de usuarios de Google Maps (provisionales) |
...
| `coches/{slug}-{id anuncio}.html` | Ficha individual de cada coche — URL ESTABLE (p.ej. `cupra-formentor-193600528.html`, `nissan-qashqai-mf168212671.html`; ver `archivo_ficha`), compartible, con Open Graph, canonical y datos estructurados schema.org/Car. El nombre antiguo `coches/{n:02d}-{slug}.html` se sigue generando como redirección (`build_redireccion_html`) para no romper enlaces ya compartidos |
| `sitemap.xml` + `robots.txt` | Los regenera `generar_web.py` en cada corrida (`escribir_sitemap_y_robots`): portada + fichas con URL estable. /alejandro/ no va en el sitemap (canonical a la versión principal) |
