...
.rd-tray-clear { background: none; border: none; color: #cfcfcf; font-size: 20px; line-height: 1; cursor: pointer; padding: 4px 2px; }
...
/* BUG encontrado: el flex estaba en .rd-cmp-wrap, pero las columnas viven
   un nivel más adentro (#rd-cmp-cols) — ese div, sin flex, quedaba en
   display:block y las tarjetas se apilaban una debajo de la otra en vez
   de quedar lado a lado. */
.rd-cmp-cols { display: flex; align-items: flex-start; }
...
/* Navegar fotos en mobile/touch: sin mouse no hay hover, y las flechas ‹ ›
   reales están pensadas para aparecer solo al pasar el mouse (opacity:0 por
   defecto) — en un dispositivo táctil eso las deja invisibles siempre. Acá
   se muestran fijas cuando el navegador ya avisa que no hay hover real. */
@media (hover: none) {
  .rd-card-nav { opacity: 1; }
}
...
(function() {
  ...
  function renderTray() {
    ...
    // Vaciar todo de una — antes había que sacar los coches de a uno.
